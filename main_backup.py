...
cap = cv2.VideoCapture(PHONE_CAMERA_URL)  # Use the default camera (0) for testing
'''
if not cap.isOpened():
...
while True:
    ...
    if fcw_primary is not None:
        ...
        # 1. IPM Ground Distance & Lateral Offset
        dist_m = fcw_primary.get("dist_m")
        lat_m = fcw_primary.get("lat_m")
        if dist_m is not None:
            lat_str = f" | Lat: {lat_m:+.1f}m" if lat_m is not None else ""
            dist_text = f"Dist: {dist_m:.1f}m{lat_str}"
        else:
            h_pct = int(fcw_primary["h_frac"] * 100)
            dist_text = f"prox: {h_pct}%"
        ...
        tau_text = f"TTC: {tau:.1f}s" if tau is not None else "TTC: ---"
